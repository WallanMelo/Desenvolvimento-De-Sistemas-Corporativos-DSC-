...
public class MecanicoDAO {
        ...
        try (Connection conn = Conexao.getConnection();
            ...
            while (rs.next()) {
                Object[] linha = {
                    ...
                    rs.getString("data_solicitacao"),
                    rs.getString("descricao"),
                    rs.getString("status")// En andamento ou Pendente
                };
                resultados.add(linha);
            }
    ...
    public static boolean iniciarManutencao(String placa, String dataConclusao) {
        String sqlManutencao = """
            UPDATE manutencoes m 
            JOIN veiculo v ON v.id = m.veiculo_id 
            SET m.status = 'Em_Andamento', m.data_conclusao = ?
            WHERE v.placa = ? AND m.status = 'Pendente'
        """;

        String sqlVeiculo = "UPDATE veiculo SET status = 'Manutenção' WHERE placa = ?";

        try (Connection conn = Conexao.getConnection()) {

            // ✅ Atualiza status da manutenção E data de conclusão
            PreparedStatement stmtManutencao = conn.prepareStatement(sqlManutencao);
            stmtManutencao.setString(1, dataConclusao); // Data de conclusão
            stmtManutencao.setString(2, placa); // Placa do veículo
            ...
            return linhasManutencao > 0 && linhasVeiculo > 0;
