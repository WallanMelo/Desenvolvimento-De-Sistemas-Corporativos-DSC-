...
public class TelaMecanico extends JFrame {
    ...
    public TelaMecanico(String nomeUsuario) {
        ...
        this.tipoUsuario = MecanicoDAO.getTipoUsuarioPorNome(nomeUsuario);
        ...
        painelMenu.setLayout(new BoxLayout(painelMenu, BoxLayout.Y_AXIS));
        painelMenu.setBackground(Color.BLACK);
        painelMenu.setBorder(BorderFactory.createEmptyBorder(10, 5, 10, 5));//evita q o text encoste na borda do panel
        ...
        //====== Botões do menu =================================================
        String[] opcoes = {
            "Manutenções Pendentes", "Histórico De Manutenções", "Solicitar Peças", "Status Da Frota", "Gerenciar Solicitações de Peças"
        };
        ...
        for (String opcao : opcoes) {
            ...
            botao.setForeground(Color.WHITE);
            botao.setBackground(new Color(80, 60, 80));
            botao.setFocusPainted(false);
            botao.setHorizontalAlignment(SwingConstants.LEFT);
            botao.setIconTextGap(10);
            botao.setCursor(Cursor.getPredefinedCursor(Cursor.HAND_CURSOR));
            ...
            botao.addActionListener(e -> {
                switch (opcao) {
                    case "Manutenções Pendentes":
                        trocarConteudo(new TelaManutencoesPendentes(this.nomeUsuario));
                        break;
                    case "Histórico De Manutenções":
                        trocarConteudo(new TelaHistoricoManutencoes());
                        break;
                    case "Solicitar Peças":
                        int usuarioId = MecanicoDAO.getUsuarioIdPorNome(this.nomeUsuario);
                        if (usuarioId != -1) {
                            trocarConteudo(new TelaSolicitarPecas(usuarioId));
                        } else {
                            JOptionPane.showMessageDialog(this, "Erro: Usuário não encontrado no banco de dados.", "Erro", JOptionPane.ERROR_MESSAGE);
                        }
                        break;
                    case "Status Da Frota":
                        trocarConteudo(new TelaStatusFrota(this.tipoUsuario, this.nomeUsuario));
                        break;
                    case "Gerenciar Solicitações de Peças":
                        trocarConteudo(new TelaListarSolicitacoesPecas(this.tipoUsuario));
                        break;
                }
            });
            painelMenu.add(botao);
    ...
    // Método para trocar o conteúdo central da page
    private void trocarConteudo(JPanel novoPainel) {
        painelConteudo.removeAll();
        painelConteudo.add(novoPainel, BorderLayout.CENTER);
        painelConteudo.revalidate();
    ...
    private JPanel criarTelaBemVindo() {
        ...
        JLabel label = new JLabel("<html><div style='text-align: center; font-size: 10px'>"
                + "Bem-vindo, Mecânico!<br>"
                + "Sua dedicação mantém nossa frota em perfeito funcionamento.<br><br>"
                + "Hoje, você possui " + quantidadeManutencoesPendentes + " manutenções pendentes e " + solicitacoesEmAnalise + " solicitações em análise. "
                + "</div></html>", SwingConstants.CENTER);
        label.setFont(new Font("Arial", Font.PLAIN, 16));
        label.setForeground(Color.BLACK);
        painel.add(label);
